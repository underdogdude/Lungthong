...
<form>
	...
	Name and Surname:<br><input type="text" name="name"/><br>
	Email:<br><input type="text" name="email"/><br>
	Confirm Email:<br><input type="text" name="emailag"/><br>
	Enter Message:<br><input type="text" name="mg"/><br>
...
<c:if test="${param.user !=null && param.pass!=null}">
	...
	<c:forEach var="i" items="${rs.rows}">
		...
		<c:if test="${stat.equals(value)}">
			<sql:update dataSource="jdbc/test" var="selected">
				INSERT INTO `test`.`member` (`username`, `password`, `name_surname`, `email`, `message`, `role`) VALUES ('${param.user}', '${param.pass}', '${param.name}', '${param.email}', '${param.mg}', 'user');
